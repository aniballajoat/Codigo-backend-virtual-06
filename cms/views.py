...
class PedidoController(CreateAPIView):
    ...
    # permission_classes sirve para 
    permission_classes = [IsAuthenticated]
    def post(self, request: Request):
        data = self.serializer_class(data=request.data)
        print(request.user)
        # request.auth => mostrara la token que se seta enviando al controlador en el apartado de headers > Authorization
        if data.is_valid():
            ...
            if documento_cliente:
                if len(documento_cliente) == 8:
                    ...
                    url = "https://apiperu.dev/api/ruc/{}".format(
                        documento_cliente)
                headers = {
                    "Authorization": "Bearer "+os.environ.get('API_PERU',''),#"6287da8da77342f7e4aab59b670dbe153f0e803c2553e7a7dcbcc7d2510ba793",
                ...
                try:
                    ...
                    with transaction.atomic():
                        ...
                        nuevoPedido = PedidoModel(pedidoTotal=0.0, 
                                                pedidoNombreCliente=dataJson.get(
                                                    'nombre_completo') if dataJson.get('nombre_completo') else dataJson.get('nombre_o_razon_social'), 
