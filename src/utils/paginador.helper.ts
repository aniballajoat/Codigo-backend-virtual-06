...
export const paginationSerializer = (total: number, query: TPaginationDto)=>{
    ...
    // si la pagina actual es mayor que uno y ademnas la pagina es menor o igual que el total de paginas, entonces la pagina previa sera la pagina actual -1, y si no sera null(no habra)
    const prevPage = page>1 && page <= totalPages ? page-1 : null;
    //  si el total de paginas es mayor que 1 y ademas la pagina es menor que el total de paginas entonces la pagina siguiente sera la pagina actual +1 y si no sera null (no habra)
